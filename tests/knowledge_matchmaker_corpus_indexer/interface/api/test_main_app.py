...
from knowledge_matchmaker_corpus_indexer.interface.api.main import app, get_container, global_container, get_global_container, main, run
...
def test_should_create_openapi_json_file():
    ...
    from knowledge_matchmaker_corpus_indexer.interface.api.main import app as rest
...
class TestMainApp:
    def test_should_have_application_title(self):
        assert_that(app.title).is_equal_to("Knowledge Matchmaker Corpus Indexer API")
    ...
    def test_should_have_ingest_route(self):
        openapi_schema = app.openapi()

        paths = openapi_schema.get("paths", {})

        assert_that(paths.get("/ingest", {})).contains("post")

    def test_should_have_get_job_route(self):
        openapi_schema = app.openapi()

        paths = openapi_schema.get("paths", {})

        assert_that(paths.get("/jobs/{job_id}", {})).contains("get")
    ...
    @patch("uvicorn.run")
    @patch("knowledge_matchmaker_corpus_indexer.interface.api.main.get_application_setting_provider")
    def test_main_function(self, mock_get_provider, mock_run):
        from knowledge_matchmaker_corpus_indexer.shared.configuration import ApplicationSettingProvider
    ...
    @patch("knowledge_matchmaker_corpus_indexer.interface.api.main.main")
