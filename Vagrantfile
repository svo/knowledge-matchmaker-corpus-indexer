...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define 'development' do |development|
    ...
    development.vm.provider :docker do |docker|
      docker.image = "svanosselaer/knowledge-matchmaker-corpus-indexer-development:latest"
      docker.has_ssh = true
      docker.pull = true
      docker.volumes = [
        '/var/run/docker.sock:/var/run/docker.sock'
      ]
      docker.ports = ["29002:8000"]
    end
    ...
    development.vm.hostname = 'knowledge-matchmaker-corpus-indexer-vagrant'
